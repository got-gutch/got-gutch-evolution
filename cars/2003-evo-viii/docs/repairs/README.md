# Repairs — 2003 Evo VIII

YouTube channel: https://www.youtube.com/@bradgutch — shorts for this car have titles starting with **"Evo 8"**

Document significant repairs here. Include the date, parts used, reason, and a link to the specific YouTube short if available.

<details>
<summary><strong>Future Repairs</strong></summary>

## FUTURE: Catalytic Converter Flange Stud Removal & Drip Molding Clip Repair

**Status:** Planned  
**Reason:** Install Tomei cat-back exhaust (currently on car with race pipe); remove factory catalytic converter studs from the two exhaust manifold flanges to allow proper cat-back installation. Additionally repair/replace drip molding clips that failed to hold after initial installation by Northfield Collision.

| Task | Details |
|------|---------|
| Cat-Back Installation | Need shop to remove studs from both exhaust manifold flanges so Tomei cat-back can be properly installed |
| Drip Molding Clip Repair | Original clips from Northfield Collision installation (3/1/26) were not holding securely. Repair or replace clips and ensure secure fit. |

**Notes:** The Tomei exhaust is currently on the car with the FB race pipe (race pipe will remain; cat-back will be added once studs are removed). Drip molding clips require investigation — may need upgraded clips or different installation method.

**References / YouTube Shorts:** https://www.youtube.com/@bradgutch *(find short titled "Evo 8 …" and add link here)*

---

## FUTURE: Front Lower Control Arm (LCA) Bushing Refresh — Powerflex Purple (Street)

**Status:** Planned  
**Technician:** Eli at Tuned Up LLC  

| Component | Part Number | Notes |
|-----------|-------------|-------|
| LCA Rear Bushing (Large) | PFF44-202G | **Highly Recommended.** Caster Adjustable / Anti-Lift Kit. This is the definitive cure for the "metallic bang" under braking. Replaces weak, oil-filled factory rubber with dual-material polyurethane that cannot bottom out. Adds Anti-Lift geometry to reduce brake dive and positive caster for improved steering feel and high-speed stability. Makes the car feel 5 years newer. |
| LCA Front Bushing (Small) | PFF44-1011-14 | Controls toe-alignment stability during hard cornering. **Verify 14mm bolt size** — earlier Evo 4/5/6 used 12mm; 14mm is standard for Evo 8/9. |
| Front Sway Bar (Optional) | PFF44-201-24 | 24mm is stock Evo 8 size. Cheap to do while the car is up. |

**Why This Fixes the "Bang":**  
The PFF44-202G kit physically cannot bottom out like the stock rubber does when it's torn. Under braking, the control arm stays exactly where it belongs, and brake pedal feel becomes much more consistent because the wheel isn't moving backward anymore.

**Recommended Action During Service:**  
With control arms off to press in bushings, have Eli inspect the lower ball joints. If original, they are likely equally worn and should be replaced at the same time.

</details>

---

## Rear Suspension Bushing Refresh — Powerflex Purple (Street)

| Field     | Value        |
|-----------|-------------|
| Date      | March 2026   |
| Technician| Eli at Tuned Up LLC |
| Parts     | Rear Upper Arm Bushings (PFR44-109 — "goo" fix), Rear Toe Arm Bushings (PFR44-115), Rear Lower Track Arm Outers (PFR44-112), Rear Lower Track Arm Inners (PFR44-111) |
| Reason    | Identified leaking bushings in the rear suspension requiring replacement with Powerflex Purple (Street) grade polyurethane for improved durability and handling. |
| Notes     | All rear Powerflex Purple components installed by Eli for a complete rear suspension refresh. Rear bushings were replaced during the same visit as the STM downpipe install on March 20, 2026. |

**Related Logs:**
- [EvoScanDataLog_2026.03.21_09.22.49.csv](../../logs/EvoScanDataLog_2026.03.21_09.22.49.csv)
- [EvoScanDataLog_2026.03.21_09.25.58.csv](../../logs/EvoScanDataLog_2026.03.21_09.25.58.csv)

**References / YouTube Shorts:** https://www.youtube.com/@bradgutch *(find short titled "Evo 8 …" and add link here)*

---

## Exhaust System Upgrade & Exterior Trim — Northfield Collision

| Field     | Value        |
|-----------|-------------|
| Date      | March 1, 2026   |
| Technician| Northfield Collision |
| Parts     | Tomei Cat-Back Exhaust with Boosted Fabrication Resonated Race Pipe, New Drip Molding |
| Reason    | Upgraded exhaust system for improved flow and performance characteristics. Replaced damaged/aged drip molding as part of exterior restoration. |
| Notes     | Tomei exhaust paired with FB resonator installed. This exhaust modification triggered P0421 (Catalyst Efficiency) code due to race pipe. Code was subsequently disabled via ECU tuning by zeroing WGDC tables and disabling rear O2 sensor monitoring bit. **Issue Found:** After driving the car a few times, the drip molding clips were not holding properly, requiring a future repair. |

**References / YouTube Shorts:** https://www.youtube.com/@bradgutch *(find short titled "Evo 8 …" and add link here)*

---

## Template

```markdown
## <Repair Title>

| Field     | Value        |
|-----------|-------------|
| Date      |              |
| Parts     |              |
| Mileage   |              |
| Reason    |              |

**References / YouTube Shorts:** https://www.youtube.com/@bradgutch *(find short titled "Evo 8 …" and add link here)*
```
